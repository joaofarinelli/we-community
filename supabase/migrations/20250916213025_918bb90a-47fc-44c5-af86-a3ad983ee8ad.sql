-- Create updated function with improved admin access
CREATE OR REPLACE FUNCTION public.can_user_see_space_v2(space_id uuid, p_user_id uuid)
RETURNS boolean AS $$
DECLARE
...
BEGIN
    ...
    SELECT EXISTS(
        ...
        WHERE space_members.space_id = can_user_see_space_v2.space_id AND user_id = p_user_id
    ...
    SELECT EXISTS(
        ...
        AND ags.space_id = can_user_see_space_v2.space_id
...
-- Update the events table policy to use the new function
...
ON public.events 
FOR SELECT 
USING ((company_id = get_user_company_id()) AND can_user_see_space_v2(space_id, auth.uid()));