...
-- Policies: view own deliveries (buyer)
CREATE POLICY IF NOT EXISTS "Buyers can view their deliveries"
...
-- Policies: user-sellers can view their deliveries
CREATE POLICY IF NOT EXISTS "User sellers can view their deliveries"
ON public.purchase_deliveries
FOR SELECT
USING (
  company_id = public.get_user_company_id()
...
-- Policies: company owners/admins can view all deliveries in their company
CREATE POLICY IF NOT EXISTS "Company owners can view all deliveries"
ON public.purchase_deliveries
FOR SELECT
USING (
...
-- Policies: company owners/admins can update deliveries (e.g., status)
CREATE POLICY IF NOT EXISTS "Company owners can update deliveries"
ON public.purchase_deliveries
FOR UPDATE
USING (
  company_id = public.get_user_company_id() AND public.is_company_owner()
