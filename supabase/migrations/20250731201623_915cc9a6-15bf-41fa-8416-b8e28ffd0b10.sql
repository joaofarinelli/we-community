-- Update remove_user_coins to handle company_id properly
...
BEGIN
  ...
  IF p_user_id IS NULL OR p_company_id IS NULL OR p_action_type IS NULL THEN
    RAISE EXCEPTION 'Invalid parameters: user_id, company_id, and action_type cannot be null';
  END IF;
