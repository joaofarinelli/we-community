-- Fix profiles table structure and create Cae Club company
DO $$
DECLARE
    new_company_id uuid;
    user_id_val uuid := 'e0f86579-0b6c-49f4-a291-5a17c6bbaf36';
BEGIN
    -- First, fix the profiles table constraints to allow multiple profiles per user
    DROP INDEX IF EXISTS profiles_user_id_key;
    ALTER TABLE public.profiles DROP CONSTRAINT IF EXISTS profiles_user_id_key;
    
    -- Add composite unique constraint for user_id + company_id
    ALTER TABLE public.profiles DROP CONSTRAINT IF EXISTS profiles_user_id_company_id_key;
    ALTER TABLE public.profiles ADD CONSTRAINT profiles_user_id_company_id_key UNIQUE (user_id, company_id);
    ...
    -- Create the owner profile for the new company
