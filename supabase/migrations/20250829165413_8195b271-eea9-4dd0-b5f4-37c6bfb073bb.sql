-- Update get_company_users_with_filters to use per-profile phone privacy
...
DECLARE
  requester_role text;
BEGIN
  -- Get requester's role in this company
  ...
  FROM public.profiles p
  LEFT JOIN public.user_tags ut ON ut.user_id = p.user_id AND ut.company_id = p_company_id
  ...
  LEFT JOIN public.user_current_level ucl ON ucl.user_id = p.user_id AND ucl.company_id = p_company_id
  LEFT JOIN public.user_levels ul ON ul.id = ucl.current_level_id
  LEFT JOIN public.user_trail_badges utb ON utb.user_id = p.user_id AND utb.company_id = p_company_id
  LEFT JOIN public.trail_badges tb ON tb.id = utb.badge_id
  ...
  HAVING (p_tag_ids IS NULL OR 
     ...
     AND (p_course_ids IS NULL OR true)
