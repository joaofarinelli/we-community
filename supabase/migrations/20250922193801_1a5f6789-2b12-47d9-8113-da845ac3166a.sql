-- Fix streak rewards system - Corrected version
...
-- Clean up duplicate streak milestone transactions by removing those with 0 coins
...
AND EXISTS (
  ...
  AND pt2.company_id = point_transactions.company_id 
  AND pt2.action_type = 'streak_milestone'
  AND pt2.coins > 0
  AND pt2.created_at::date = point_transactions.created_at::date
);

-- Recalculate user points totals to fix any inconsistencies from existing data
