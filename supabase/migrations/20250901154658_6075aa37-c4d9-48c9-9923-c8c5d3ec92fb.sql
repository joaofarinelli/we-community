-- Fix and create the get_company_users_with_filters RPC function
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(user_posts.posts_count, 0) as posts_count,
  ...
  LEFT JOIN (
    SELECT 
      posts.created_by as user_id,
      COUNT(*) as posts_count
    FROM public.posts
    WHERE posts.company_id = p_company_id
    GROUP BY posts.created_by
  ) user_posts ON user_posts.user_id = p.user_id
...
CREATE INDEX IF NOT EXISTS idx_user_tags_user_company ON public.user_tags(user_id, company_id);
CREATE INDEX IF NOT EXISTS idx_user_current_level_user_company ON public.user_current_level(user_id, company_id);
CREATE INDEX IF NOT EXISTS idx_posts_created_by_company ON public.posts(created_by, company_id);
CREATE INDEX IF NOT EXISTS idx_user_course_progress_user_course ON public.user_course_progress(user_id, course_id);
CREATE INDEX IF NOT EXISTS idx_user_trail_badges_user_company ON public.user_trail_badges(user_id, company_id);