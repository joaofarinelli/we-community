-- Update the create_course_secure function to handle linear_module_progression parameter
CREATE OR REPLACE FUNCTION public.create_course_secure(
  ...
  p_linear_module_progression boolean DEFAULT false,
  p_mentor_name text DEFAULT NULL::text, 
  p_mentor_role text DEFAULT NULL::text, 
  p_mentor_signature_url text DEFAULT NULL::text, 
  p_certificate_background_url text DEFAULT NULL::text, 
  p_certificate_footer_text text DEFAULT NULL::text, 
  p_access_criteria jsonb DEFAULT '{}'::jsonb
)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
