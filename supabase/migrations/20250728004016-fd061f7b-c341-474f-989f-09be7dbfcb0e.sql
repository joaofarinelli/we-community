-- Fix the UUID casting and merge the profiles properly
...
SELECT DISTINCT sm.space_id, '8beb8843-aeb0-4afd-a6b5-2bd90c2a1731'::uuid, sm.role, sm.joined_at
...
WHERE sm.user_id = 'e0f86579-0b6c-49f4-a291-5a17c6bbaf36'::uuid
  AND NOT EXISTS (
    ...
    WHERE sm2.space_id = sm.space_id 
      AND sm2.user_id = '8beb8843-aeb0-4afd-a6b5-2bd90c2a1731'::uuid
  );

-- Remove space memberships for the newer user_id
DELETE FROM space_members WHERE user_id = 'e0f86579-0b6c-49f4-a291-5a17c6bbaf36'::uuid;

-- Remove the newer duplicate profile
DELETE FROM public.profiles 
WHERE user_id = 'e0f86579-0b6c-49f4-a291-5a17c6bbaf36'::uuid 
  AND email = 'jv.farinelli@gmail.com' 
  AND company_id = '1e184d5f-db73-4236-a408-1b84b73cbe34'::uuid;

-- Verify cleanup - should only show one profile now
