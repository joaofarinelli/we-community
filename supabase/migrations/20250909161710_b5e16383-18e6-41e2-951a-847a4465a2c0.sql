-- Fix issue_course_certificate function to use auth.uid() instead of 'system' for issued_by
...
BEGIN
  ...
  -- Insert certificate using auth.uid() for issued_by instead of 'system'
  INSERT INTO public.user_course_certificates (
    ...
    v_duration_minutes, now(), auth.uid(),
