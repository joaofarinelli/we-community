...
-- Update companies RLS policy to allow domain-based access
DROP POLICY IF EXISTS "Companies are viewable by members" ON public.companies;

CREATE POLICY "Companies are viewable by domain or members" ON public.companies
FOR SELECT USING (
  ...
  OR 
  -- Allow anonymous access to active companies for domain lookup
  (status = 'active')
);
