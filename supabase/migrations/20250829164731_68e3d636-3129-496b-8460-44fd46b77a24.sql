-- Update the user filtering function to respect phone privacy settings
...
BEGIN
  ...
  -- Get requesting user's role in this company
  SELECT pr.role INTO requesting_user_role
  FROM profiles pr
  WHERE pr.user_id = requesting_user_id AND pr.company_id = p_company_id;
  ...
  SELECT 
    ...
    pr.last_name,
    pr.email,
    -- Hide phone if user has privacy setting enabled and requester is not owner/admin
    CASE 
      WHEN pr.hide_phone_from_members = true 
           AND requesting_user_role NOT IN ('owner', 'admin')
           AND pr.user_id != requesting_user_id
      THEN NULL
      ELSE pr.phone
    END as phone,
    pr.role,
