...
DROP FUNCTION IF EXISTS public.provision_onboarding_assignment(uuid, uuid);

-- Create updated provision_onboarding_assignment function with jsonb return type
...
BEGIN
  ...
  IF existing_assignment_id IS NOT NULL THEN
    ...
    LOOP
      INSERT INTO public.onboarding_step_progress (
        ...
        existing_assignment_id, step_record.id, 'pending', '{}'::jsonb
      )
  ...
  LOOP
    INSERT INTO public.onboarding_step_progress (
      ...
      new_assignment_id, step_record.id, 'pending', '{}'::jsonb
    );
...
BEGIN
  ...
  LOOP
    INSERT INTO public.onboarding_step_progress (
      ...
      p_assignment_id, step_record.id, 'pending', '{}'::jsonb
    )
    ON CONFLICT (assignment_id, step_id) DO NOTHING;
  ...
  LOOP
    ...
    LOOP
      ...
      INSERT INTO public.onboarding_step_progress (
        ...
        assignment_record.id, step_record.id, 'pending', '{}'::jsonb
      )
      ON CONFLICT (assignment_id, step_id) DO NOTHING;
